...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.EphedrineField.coords, true) < 100		then
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.EphedrineProcessing.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowHelpNotification(_U('ephedrine_processprompt'))
			end
				...
				if Config.LicenseEnable then
					ESX.TriggerServerCallback('esx_license:checkLicense', function(hasProcessingLicense)
						if hasProcessingLicense then
							...
							OpenBuyLicenseMenu('ephedrine_processing')
						end
					end, GetPlayerServerId(PlayerId()), 'ephedrine_processing')
				else
					ProcessEphedra()
				end
...
function ProcessEphedra()
	...
	ESX.ShowNotification(_U('ephedrine_processingstarted'))
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.EphedrineProcessing.coords, false) > 4 then
			ESX.ShowNotification(_U('ephedrine_processingtoofar'))
...
function ValidateEphedraCoord(plantCoord)
	if spawnedEphedra > 0 then
		...
		if GetDistanceBetweenCoords(plantCoord, Config.CircleZones.EphedrineField.coords, false) > 50 then
			validate = false
		end
...
function GenerateEphedraCoords()
	while true do
		...
		ephedraCoordX = Config.CircleZones.EphedrineField.coords.x + modX
		ephedraCoordY = Config.CircleZones.EphedrineField.coords.y + modY
